...
rect.setSize({w,h}); rect.setPosition(pos); rect.setFillColor(sf::Color(50,50,50,200));
label.setFont(font); label.setString(text); label.setCharacterSize(24);
sf::FloatRect tb = label.getLocalBounds();
label.setPosition(pos.x + (w - tb.width)/2 - tb.left, pos.y + (h - tb.height)/2 - tb.top);
}
